...
export function Sidebar({ onOpenDiagram, activeDiagramId, onRefresh }: Props) {
  ...
  async function buildTagItems(d: Diagram): Promise<ContextMenuItem[]> {
    ...
    return allTags.map(t => ({
      ...
      icon: <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: t.color }} />,
        ...
        <div className="flex-1 min-w-0">
          {editingId === d.id ? (
            <input autoFocus value={editValue} onChange={e => setEditValue(e.target.value)}
              ...
              className="w-full text-xs bg-transparent border-b outline-none py-0"
    ...
    return (
      <div key={f.id}>
        <div className="group flex items-center gap-1 py-[5px] rounded-md cursor-pointer select-none transition-colors duration-100"
          ...
          <span className="flex-shrink-0 opacity-70">{isOpen ? <FolderOpen size={13} /> : <Folder size={13} />}</span>
          {editingId === f.id ? (
            <input autoFocus value={editValue} onChange={e => setEditValue(e.target.value)}
              onBlur={() => commitEdit(f.id, 'folder')}
              onKeyDown={e => { if (e.key === 'Enter') {commitEdit(f.id, 'folder');} if (e.key === 'Escape') {setEditingId(null);} }}
              onClick={e => e.stopPropagation()}
              className="flex-1 text-xs bg-transparent border-b outline-none py-0"
              style={{ borderColor: 'var(--accent)', color: 'var(--text-primary)' }} />
          ) : (
  ...
  return (
    <div className="flex flex-col h-full border-r" style={{ background: 'var(--surface-raised)', borderColor: 'var(--border-subtle)' }}>
      ...
      <div className="px-2 py-2 border-b flex-shrink-0" style={{ borderColor: 'var(--border-subtle)' }}>
        <div className="relative">
          ...
          <input ref={searchRef} type="text" value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="w-full pl-7 pr-7 py-1.5 text-xs rounded-lg border outline-none transition-colors"
            style={{
      ...
      {tags.length > 0 && (
        <div className="px-2 py-1.5 border-b flex-shrink-0 flex flex-wrap gap-1 items-center" style={{ borderColor: 'var(--border-subtle)' }}>
          {tags.map(t => (
      ...
      {showNewTag && (
        <div className="px-2 py-2 border-b flex-shrink-0 space-y-1.5 animate-fade-in" style={{ borderColor: 'var(--border-subtle)' }}>
          ...
          <div className="flex gap-1">
            <button onClick={handleCreateTag} className="px-2 py-1 text-[10px] font-medium rounded text-white" style={{ background: 'var(--accent)' }}>{t('common.save')}</button>
            <button onClick={() => setShowNewTag(false)} className="px-2 py-1 text-[10px] rounded" style={{ color: 'var(--text-secondary)' }}>{t('common.cancel')}</button>
          </div>
        </div>
      )}

      {selectedIds.size > 0 && (
        <div className="px-2 py-2 border-b flex-shrink-0 animate-fade-in" style={{ borderColor: 'var(--border-subtle)', background: 'var(--accent-dim)' }}>
          ...
          <div className="flex gap-1">
            <button onClick={() => setDeleteConfirm({ ids: Array.from(selectedIds) })}
              className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 text-[10px] font-medium rounded text-white"
            ...
            <button onClick={() => { setPickerDiagramIds(Array.from(selectedIds)); setShowFolderPicker(true); }}
              className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 text-[10px] font-medium rounded border"
      ...
      {tags.length === 0 && (
        <div className="px-2 py-1.5 border-b flex-shrink-0" style={{ borderColor: 'var(--border-subtle)' }}>
          <button onClick={() => setShowNewTag(true)}
            className="flex items-center gap-1.5 text-[10px] w-full px-2 py-1 rounded transition-colors hover:bg-white/5"
            style={{ color: 'var(--text-tertiary)' }}>
