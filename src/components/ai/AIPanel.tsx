...
function CodeBlock({
  ...
  const validateCode = (mermaidCode: string): boolean => {
    ...
    const startsValidly = validStarts.some(start =>
      firstLine.toLowerCase().startsWith(start.toLowerCase())
    );
  ...
  return (
    <div
      className="w-full my-2 rounded-xl overflow-hidden border"
    ...
    >
      ...
      <pre
        className="px-3 py-2.5 text-[11px] font-mono leading-relaxed overflow-x-auto"
...
function Bubble({ msg, onApply }: { msg: AIMessage; onApply?: (c: string) => void }) {
  ...
  const showFallbackApply = !isUser && !hasCodeBlock && looksLikeMermaid(msg.content) && !!onApply;

  return (
    <div
      data-testid={isUser ? 'ai-message-user' : 'ai-response'}
      ...
      <div className={`flex-1 min-w-0 ${isUser ? 'flex flex-col items-end' : ''}`}>
        {parts.map((p, i) => {
          ...
          return (
            <div
              key={i}
              className={`px-3 py-2 rounded-2xl text-xs leading-relaxed mb-1 ai-bubble ${isUser ? 'ml-8' : 'mr-4'}`}
              style={{
                background: isUser ? 'var(--accent)' : 'var(--surface-floating)',
                color: isUser ? '#fff' : 'var(--text-primary)',
              }}
            >
              <Markdown>{p.trim()}</Markdown>
            </div>
          );
        })}
        {showFallbackApply && (
          <button
            onClick={() => onApply?.(extractMermaidCode(msg.content))}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium text-white mr-4 mb-1 transition-all hover:opacity-90"
...
export function AIPanel({
  ...
  const { machineSize, isConfigured, config } = useAISettings(settingsKey);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-4">
        ...
        {messages.map(m => (
          <Bubble key={m.id} msg={m} onApply={onApply} />
        ))}
        {loading && fixMode && (
          ...
          <div className="flex gap-2.5">
            ...
            <div className="flex flex-col gap-1.5 flex-1 min-w-0">
              <div
                ...
                style={{ background: 'var(--surface-floating)', alignSelf: 'flex-start' }}
              >
